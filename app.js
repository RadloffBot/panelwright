...
(function (global) {
  ...
  function panelCSVRows(panel) {
    ...
    if (t.neutralLimit != null && t.is3ph) L.push(['Neutral Limit 5% (A, screening guideline — not a NEC limit)', t.neutralLimit]);
    return L;
  }
  ...
  function panelReportHTML(p) {
    ...
    return '<section class="pr-sec pr-panel">' +
      ...
      (t.is3ph ? ' · Neutral ≈' + t.neutralEst + ' A' + (t.neutralLimit != null ? ' (screening limit ' + t.neutralLimit + ' A, 5% guideline — not a NEC limit)' : '') + (t.neutralOk === false ? ' — <strong>above the 5% screening guideline</strong>' : '') : '') +
      '</p></section>';
  }
  ...
  function printReportHTML(project, date) {
    ...
    out.push('<div class="pr-foot">' +
      '<p><strong>Design aid only.</strong> Not a substitute for the NEC, manufacturer instructions, or a qualified electrical design. Sizing follows NEC 210.20(A)/215.2(A) continuous-load rules and 240.6 standard sizes; balancing uses IEEE-style max deviation plus a 5% neutral-load screening guideline (industry practice, field approximation — the NEC sets no percent-unbalance limit on panelboards; neutral-conductor minimum ampacity is governed by 220.61 / 310.12(D)). Verify against the adopted NEC edition and local code.</p>' +
  ...
  function renderBadges() {
    ...
    $('#badges').innerHTML = `
      ...
      ${warnNeutral ? '<span class="badge bad">Above 5% neutral screening guideline (not a NEC limit)</span>' : ''}`;
  }
